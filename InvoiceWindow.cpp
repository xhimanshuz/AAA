...
InvoiceWindow::InvoiceWindow(const QString _client, const QStringList invno, const int rono, QWidget *parent) : QDialog(parent), islastSelectedMedia(false)
{
    ...
    if(!invno.isEmpty())
        generateBill->setValueFromNumbers(invno);
...
void InvoiceWindow::setupSignals()
{
    connect(billListView, &QTableView::clicked, [this](const QModelIndex &index){
        ...
        if(invoice > 0)
            generateBill->setValue(io->sql->getInvoiceList(invoice));
        else
    ...
    connect(generateBill, &AddInvoice::saveClicked, this, &InvoiceWindow::populateData);
}
